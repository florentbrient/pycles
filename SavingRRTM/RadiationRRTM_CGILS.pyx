...
class RadiationRRTM_CGILS: # No change currently, should use the reference profile to extend the calculation.
    ...
    def update_calculate(self,grid,basicstate,scalars,velocities,thermodynamics,surface,comm,io,timestepping):
        ...
        cdef double [:] lw_up_flux_ext    = np.zeros(nz+ n_ext+1,dtype=np.double,order='c')
        cdef double [:] lw_down_flux_ext  = np.zeros(nz+ n_ext+1,dtype=np.double,order='c')
        cdef double [:] sw_up_flux_ext    = np.zeros(nz+ n_ext+1,dtype=np.double,order='c')
        cdef double [:] sw_down_flux_ext  = np.zeros(nz+ n_ext+1,dtype=np.double,order='c')
        cdef double [:] lw_up_fluxc_ext   = np.zeros(nz+ n_ext+1,dtype=np.double,order='c')
        cdef double [:] lw_down_fluxc_ext = np.zeros(nz+ n_ext+1,dtype=np.double,order='c')
        cdef double [:] sw_up_fluxc_ext   = np.zeros(nz+ n_ext+1,dtype=np.double,order='c')
        cdef double [:] sw_down_fluxc_ext = np.zeros(nz+ n_ext+1,dtype=np.double,order='c')
        cdef double [:] lw_tt_ext    = np.zeros(nz+ n_ext,dtype=np.double,order='c')
        cdef double [:] sw_tt_ext    = np.zeros(nz+ n_ext,dtype=np.double,order='c')
        cdef double [:] total_tt_ext = np.zeros(nz+ n_ext,dtype=np.double,order='c')
        cdef double [:] lw_ttc_ext    = np.zeros(nz+ n_ext,dtype=np.double,order='c')
        cdef double [:] sw_ttc_ext    = np.zeros(nz+ n_ext,dtype=np.double,order='c')
        cdef double [:] total_ttc_ext = np.zeros(nz+ n_ext,dtype=np.double,order='c')
