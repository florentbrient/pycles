...
! by scaling mass mixing ratio (g/g) with the appropriate molecular weights (g/mol) 
      ...
      real(kind=8), intent(in) :: o2vmr(:,:)         ! Oxygen volume mixing ratio
                                                      !    Dimensions: (ncol,nlay)
      real(kind=8), intent(in) :: asdir(:)           ! UV/vis surface albedo direct rad
                                                      !    Dimensions: (ncol)
      real(kind=8), intent(in) :: aldir(:)           ! Near-IR surface albedo direct rad
                                                      !    Dimensions: (ncol)
      real(kind=8), intent(in) :: asdif(:)           ! UV/vis surface albedo: diffuse rad
                                                      !    Dimensions: (ncol)
      real(kind=8), intent(in) :: aldif(:)           ! Near-IR surface albedo: diffuse rad
                                                      !    Dimensions: (ncol)
      ...
      real(kind=8), intent(in) :: coszen(:)          ! Cosine of solar zenith angle
                                                      !    Dimensions: (ncol)
      ...
      real(kind=8), intent(in) :: asmcmcl(:,:,:)     ! In-cloud asymmetry parameter
                                                      !    Dimensions: (ngptsw,ncol,nlay)
      real(kind=8), intent(in) :: fsfcmcl(:,:,:)     ! In-cloud forward scattering fraction
                                                      !    Dimensions: (ngptsw,ncol,nlay)
      real(kind=8), intent(in) :: ciwpmcl(:,:,:)     ! In-cloud ice water path (g/m2)
                                                      !    Dimensions: (ngptsw,ncol,nlay)
      real(kind=8), intent(in) :: clwpmcl(:,:,:)     ! In-cloud liquid water path (g/m2)
                                                      !    Dimensions: (ngptsw,ncol,nlay)
      real(kind=8), intent(in) :: reicmcl(:,:)       ! Cloud ice effective radius (microns)
                                                      !    Dimensions: (ncol,nlay)
      ...
      real(kind=8), intent(in) :: relqmcl(:,:)       ! Cloud water drop effective radius (microns)
                                                      !    Dimensions: (ncol,nlay)
      real(kind=8), intent(in) :: tauaer(:,:,:)      ! Aerosol optical depth (iaer=10 only)
                                                      !    Dimensions: (ncol,nlay,nbndsw)
                                                      ! (non-delta scaled)      
      real(kind=8), intent(in) :: ssaaer(:,:,:)      ! Aerosol single scattering albedo (iaer=10 only)
                                                      !    Dimensions: (ncol,nlay,nbndsw)
                                                      ! (non-delta scaled)      
      real(kind=8), intent(in) :: asmaer(:,:,:)      ! Aerosol asymmetry parameter (iaer=10 only)
                                                      !    Dimensions: (ncol,nlay,nbndsw)
                                                      ! (non-delta scaled)      
      real(kind=8), intent(in) :: ecaer(:,:,:)       ! Aerosol optical depth at 0.55 micron (iaer=6 only)
                                                      !    Dimensions: (ncol,nlay,naerec)
                                                      ! (non-delta scaled)      

! ----- Output -----

      real(kind=8), intent(out) :: swuflx(:,:)       ! Total sky shortwave upward flux (W/m2)
                                                      !    Dimensions: (ncol,nlay+1)
      real(kind=8), intent(out) :: swdflx(:,:)       ! Total sky shortwave downward flux (W/m2)
                                                      !    Dimensions: (ncol,nlay+1)
      real(kind=8), intent(out) :: swhr(:,:)         ! Total sky shortwave radiative heating rate (K/d)
                                                      !    Dimensions: (ncol,nlay)
      real(kind=8), intent(out) :: swuflxc(:,:)      ! Clear sky shortwave upward flux (W/m2)
                                                      !    Dimensions: (ncol,nlay+1)
      real(kind=8), intent(out) :: swdflxc(:,:)      ! Clear sky shortwave downward flux (W/m2)
                                                      !    Dimensions: (ncol,nlay+1)
      real(kind=8), intent(out) :: swhrc(:,:)        ! Clear sky shortwave radiative heating rate (K/d)
                                                      !    Dimensions: (ncol,nlay)
...
! Atmosphere
      ...
      real(kind=8) :: pdp(nlay+1)            ! layer pressure thickness (hPa, mb)
      real(kind=8) :: coldry(nlay+1)         ! dry air column amount
...
!      real(kind=8) :: earth_sun             ! function for Earth/Sun distance factor
      ...
      real(kind=8) :: taua(nlay+1,nbndsw)    ! Aerosol optical depth
      real(kind=8) :: ssaa(nlay+1,nbndsw)    ! Aerosol single scattering albedo
      real(kind=8) :: asma(nlay+1,nbndsw)    ! Aerosol asymmetry parameter
...
! Atmosphere - setcoef
      ...
      integer(kind=4) :: jp(nlay+1)          ! 
      integer(kind=4) :: jt(nlay+1)          !
      integer(kind=4) :: jt1(nlay+1)         !
      ...
      real(kind=8) :: colmol(nlay+1)         ! column amount
      real(kind=8) :: co2mult(nlay+1)        ! column amount 
...
!      real(kind=8) :: cldfrac(nlay+1)        ! layer cloud fraction
!      real(kind=8) :: tauc(nlay+1)           ! in-cloud optical depth (non-delta scaled)
!      real(kind=8) :: ssac(nlay+1)           ! in-cloud single scattering albedo (non-delta scaled)
!      real(kind=8) :: asmc(nlay+1)           ! in-cloud asymmetry parameter (non-delta scaled)
!      real(kind=8) :: fsfc(nlay+1)           ! in-cloud forward scattering fraction (non-delta scaled)
!      real(kind=8) :: ciwp(nlay+1)           ! in-cloud ice water path
!      real(kind=8) :: clwp(nlay+1)           ! in-cloud liquid water path
!      real(kind=8) :: rei(nlay+1)            ! cloud ice particle size
!      real(kind=8) :: rel(nlay+1)            ! cloud liquid particle size

!      real(kind=8) :: taucloud(nlay+1,jpband)  ! in-cloud optical depth
!      real(kind=8) :: taucldorig(nlay+1,jpband)! in-cloud optical depth (non-delta scaled)
!      real(kind=8) :: ssacloud(nlay+1,jpband)  ! in-cloud single scattering albedo
!      real(kind=8) :: asmcloud(nlay+1,jpband)  ! in-cloud asymmetry parameter

! Atmosphere/clouds - cldprmc [mcica]
      real(kind=8) :: cldfmc(ngptsw,nlay+1)    ! cloud fraction [mcica]
      real(kind=8) :: ciwpmc(ngptsw,nlay+1)    ! in-cloud ice water path [mcica]
      real(kind=8) :: clwpmc(ngptsw,nlay+1)    ! in-cloud liquid water path [mcica]
      ...
      real(kind=8) :: taucmc(ngptsw,nlay+1)    ! in-cloud optical depth [mcica]
      real(kind=8) :: taormc(ngptsw,nlay+1)    ! unscaled in-cloud optical depth [mcica]
      real(kind=8) :: ssacmc(ngptsw,nlay+1)    ! in-cloud single scattering albedo [mcica]
      real(kind=8) :: asmcmc(ngptsw,nlay+1)    ! in-cloud asymmetry parameter [mcica]
      real(kind=8) :: fsfcmc(ngptsw,nlay+1)    ! in-cloud forward scattering fraction [mcica]

! Atmosphere/clouds/aerosol - spcvrt,spcvmc
      real(kind=8) :: ztauc(nlay+1,nbndsw)     ! cloud optical depth
      real(kind=8) :: ztaucorig(nlay+1,nbndsw) ! unscaled cloud optical depth
      real(kind=8) :: zasyc(nlay+1,nbndsw)     ! cloud asymmetry parameter 
                                                !  (first moment of phase function)
      real(kind=8) :: zomgc(nlay+1,nbndsw)     ! cloud single scattering albedo
      real(kind=8) :: ztaua(nlay+1,nbndsw)     ! total aerosol optical depth
      real(kind=8) :: zasya(nlay+1,nbndsw)     ! total aerosol asymmetry parameter 
      real(kind=8) :: zomga(nlay+1,nbndsw)     ! total aerosol single scattering albedo

      real(kind=8) :: zcldfmc(nlay+1,ngptsw)   ! cloud fraction [mcica]
      real(kind=8) :: ztaucmc(nlay+1,ngptsw)   ! cloud optical depth [mcica]
      real(kind=8) :: ztaormc(nlay+1,ngptsw)   ! unscaled cloud optical depth [mcica]
      real(kind=8) :: zasycmc(nlay+1,ngptsw)   ! cloud asymmetry parameter [mcica] 
      real(kind=8) :: zomgcmc(nlay+1,ngptsw)   ! cloud single scattering albedo [mcica]

      real(kind=8) :: zbbfu(nlay+2)          ! temporary upward shortwave flux (w/m2)
      real(kind=8) :: zbbfd(nlay+2)          ! temporary downward shortwave flux (w/m2)
      real(kind=8) :: zbbcu(nlay+2)          ! temporary clear sky upward shortwave flux (w/m2)
      real(kind=8) :: zbbcd(nlay+2)          ! temporary clear sky downward shortwave flux (w/m2)
      real(kind=8) :: zbbfddir(nlay+2)       ! temporary downward direct shortwave flux (w/m2)
      real(kind=8) :: zbbcddir(nlay+2)       ! temporary clear sky downward direct shortwave flux (w/m2)
      real(kind=8) :: zuvfd(nlay+2)          ! temporary UV downward shortwave flux (w/m2)
      real(kind=8) :: zuvcd(nlay+2)          ! temporary clear sky UV downward shortwave flux (w/m2)
      real(kind=8) :: zuvfddir(nlay+2)       ! temporary UV downward direct shortwave flux (w/m2)
      real(kind=8) :: zuvcddir(nlay+2)       ! temporary clear sky UV downward direct shortwave flux (w/m2)
      real(kind=8) :: znifd(nlay+2)          ! temporary near-IR downward shortwave flux (w/m2)
      real(kind=8) :: znicd(nlay+2)          ! temporary clear sky near-IR downward shortwave flux (w/m2)
      real(kind=8) :: znifddir(nlay+2)       ! temporary near-IR downward direct shortwave flux (w/m2)
      real(kind=8) :: znicddir(nlay+2)       ! temporary clear sky near-IR downward direct shortwave flux (w/m2)

! Optional output fields 
      real(kind=8) :: swnflx(nlay+2)         ! Total sky shortwave net flux (W/m2)
      real(kind=8) :: swnflxc(nlay+2)        ! Clear sky shortwave net flux (W/m2)
      real(kind=8) :: dirdflux(nlay+2)       ! Direct downward shortwave surface flux
      real(kind=8) :: difdflux(nlay+2)       ! Diffuse downward shortwave surface flux
      real(kind=8) :: uvdflx(nlay+2)         ! Total sky downward shortwave flux, UV/vis  
      real(kind=8) :: nidflx(nlay+2)         ! Total sky downward shortwave flux, near-IR 
      real(kind=8) :: dirdnuv(nlay+2)        ! Direct downward shortwave flux, UV/vis
      real(kind=8) :: difdnuv(nlay+2)        ! Diffuse downward shortwave flux, UV/vis
      real(kind=8) :: dirdnir(nlay+2)        ! Direct downward shortwave flux, near-IR
      real(kind=8) :: difdnir(nlay+2)        ! Diffuse downward shortwave flux, near-IR

! Output - inactive
!      real(kind=8) :: zuvfu(nlay+2)         ! temporary upward UV shortwave flux (w/m2)
!      real(kind=8) :: zuvfd(nlay+2)         ! temporary downward UV shortwave flux (w/m2)
!      real(kind=8) :: zuvcu(nlay+2)         ! temporary clear sky upward UV shortwave flux (w/m2)
!      real(kind=8) :: zuvcd(nlay+2)         ! temporary clear sky downward UV shortwave flux (w/m2)
!      real(kind=8) :: zvsfu(nlay+2)         ! temporary upward visible shortwave flux (w/m2)
!      real(kind=8) :: zvsfd(nlay+2)         ! temporary downward visible shortwave flux (w/m2)
!      real(kind=8) :: zvscu(nlay+2)         ! temporary clear sky upward visible shortwave flux (w/m2)
!      real(kind=8) :: zvscd(nlay+2)         ! temporary clear sky downward visible shortwave flux (w/m2)
!      real(kind=8) :: znifu(nlay+2)         ! temporary upward near-IR shortwave flux (w/m2)
!      real(kind=8) :: znifd(nlay+2)         ! temporary downward near-IR shortwave flux (w/m2)
!      real(kind=8) :: znicu(nlay+2)         ! temporary clear sky upward near-IR shortwave flux (w/m2)
!      real(kind=8) :: znicd(nlay+2)         ! temporary clear sky downward near-IR shortwave flux (w/m2)
...
! by scaling mass mixing ratio (g/g) with the appropriate molecular weights (g/mol) 
      ...
      real(kind=8), intent(in) :: cldfmcl(:,:,:)     ! Cloud fraction
                                                      ! Dimensions: (ngptsw,ncol,nlay)
      real(kind=8), intent(in) :: taucmcl(:,:,:)     ! In-cloud optical depth (optional)
                                                      ! Dimensions: (ngptsw,ncol,nlay)
      real(kind=8), intent(in) :: ssacmcl(:,:,:)     ! In-cloud single scattering albedo
                                                      ! Dimensions: (ngptsw,ncol,nlay)
      real(kind=8), intent(in) :: asmcmcl(:,:,:)     ! In-cloud asymmetry parameter
...
! Atmosphere
      ...
      real(kind=8), intent(out) :: pdp(:)            ! layer pressure thickness (hPa, mb)
                                                      ! Dimensions: (nlay)
      ...
      real(kind=8), intent(out) :: adjflux(:)        ! adjustment for current Earth/Sun distance
                                                      ! Dimensions: (jpband)
      real(kind=8), intent(out) :: solvar(:)         ! solar constant scaling factor from rrtmg_sw
                                                      ! Dimensions: (jpband)
                                                      !  default value of 1368.22 Wm-2 at 1 AU
      real(kind=8), intent(out) :: taua(:,:)         ! Aerosol optical depth
                                                      ! Dimensions: (nlay,nbndsw)
      real(kind=8), intent(out) :: ssaa(:,:)         ! Aerosol single scattering albedo
                                                      ! Dimensions: (nlay,nbndsw)
      real(kind=8), intent(out) :: asma(:,:)         ! Aerosol asymmetry parameter
                                                      ! Dimensions: (nlay,nbndsw)
...
! Atmosphere/clouds - cldprmc
      ...
      real(kind=8), intent(out) :: cldfmc(:,:)       ! layer cloud fraction
                                                      ! Dimensions: (ngptsw,nlay)
      real(kind=8), intent(out) :: taucmc(:,:)       ! in-cloud optical depth (non-delta scaled)
                                                      ! Dimensions: (ngptsw,nlay)
      real(kind=8), intent(out) :: ssacmc(:,:)       ! in-cloud single scattering albedo (non-delta-scaled)
                                                      ! Dimensions: (ngptsw,nlay)
      real(kind=8), intent(out) :: asmcmc(:,:)       ! in-cloud asymmetry parameter (non-delta scaled)
                                                      ! Dimensions: (ngptsw,nlay)
      real(kind=8), intent(out) :: fsfcmc(:,:)       ! in-cloud forward scattering fraction (non-delta scaled)
                                                      ! Dimensions: (ngptsw,nlay)
      real(kind=8), intent(out) :: ciwpmc(:,:)       ! in-cloud ice water path
                                                      ! Dimensions: (ngptsw,nlay)
      real(kind=8), intent(out) :: clwpmc(:,:)       ! in-cloud liquid water path
                                                      ! Dimensions: (ngptsw,nlay)
...
!  e.g. h2ovmr = h2ommr * amdw)
      ...
      real(kind=8), parameter :: sbc = 5.67e-08_8   ! Stefan-Boltzmann constant (W/m2K4)
...
!         ssacmc(:,nlayers) = 1.0_8
!         asmcmc(:,nlayers) = 0.0_8
!         fsfcmc(:,nlayers) = 0.0_8
