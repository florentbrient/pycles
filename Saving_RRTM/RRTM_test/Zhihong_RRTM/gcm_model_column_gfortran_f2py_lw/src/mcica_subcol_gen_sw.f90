...
! ----- Input -----
! Control
      integer(kind=4), intent(in) :: iplon           ! column/longitude dimension
      integer(kind=4), intent(in) :: ncol            ! number of columns
      integer(kind=4), intent(in) :: nlay            ! number of model layers
      integer(kind=4), intent(in) :: icld            ! clear/cloud, cloud overlap flag
      integer(kind=4), intent(in) :: permuteseed     ! if the cloud generator is called multiple times,
                                                      ! permute the seed between each call;
                                                      ! between calls for LW and SW, recommended
                                                      ! permuteseed differs by 'ngpt'
      integer(kind=4), intent(inout) :: irng         ! flag for random number generator
                                                      !  0 = kissvec
                                                      !  1 = Mersenne Twister
        
! Atmosphere
      real(kind=8), intent(in) :: play(:,:)          ! layer pressures (mb) 
                                                      !    Dimensions: (ncol,nlay)

! Atmosphere/clouds - cldprop
      real(kind=8), intent(in) :: cldfrac(:,:)       ! layer cloud fraction
                                                      !    Dimensions: (ncol,nlay)
      real(kind=8), intent(in) :: tauc(:,:,:)        ! in-cloud optical depth
                                                      !    Dimensions: (nbndsw,ncol,nlay)
      real(kind=8), intent(in) :: ssac(:,:,:)        ! in-cloud single scattering albedo (non-delta scaled)
                                                      !    Dimensions: (nbndsw,ncol,nlay)
      real(kind=8), intent(in) :: asmc(:,:,:)        ! in-cloud asymmetry parameter (non-delta scaled)
                                                      !    Dimensions: (nbndsw,ncol,nlay)
      real(kind=8), intent(in) :: fsfc(:,:,:)        ! in-cloud forward scattering fraction (non-delta scaled)
                                                      !    Dimensions: (nbndsw,ncol,nlay)
      real(kind=8), intent(in) :: ciwp(:,:)          ! in-cloud ice water path
                                                      !    Dimensions: (ncol,nlay)
      real(kind=8), intent(in) :: clwp(:,:)          ! in-cloud liquid water path
                                                      !    Dimensions: (ncol,nlay)
      real(kind=8), intent(in) :: rei(:,:)           ! cloud ice particle size
                                                      !    Dimensions: (ncol,nlay)
      real(kind=8), intent(in) :: rel(:,:)           ! cloud liquid particle size
                                                      !    Dimensions: (ncol,nlay)

! ----- Output -----
! Atmosphere/clouds - cldprmc [mcica]
      real(kind=8), intent(out) :: cldfmcl(:,:,:)    ! cloud fraction [mcica]
                                                      !    Dimensions: (ngptsw,ncol,nlay)
      real(kind=8), intent(out) :: ciwpmcl(:,:,:)    ! in-cloud ice water path [mcica]
                                                      !    Dimensions: (ngptsw,ncol,nlay)
      real(kind=8), intent(out) :: clwpmcl(:,:,:)    ! in-cloud liquid water path [mcica]
                                                      !    Dimensions: (ngptsw,ncol,nlay)
      real(kind=8), intent(out) :: relqmcl(:,:)      ! liquid particle size (microns)
                                                      !    Dimensions: (ncol,nlay)
      real(kind=8), intent(out) :: reicmcl(:,:)      ! ice partcle size (microns)
                                                      !    Dimensions: (ncol,nlay)
      real(kind=8), intent(out) :: taucmcl(:,:,:)    ! in-cloud optical depth [mcica]
                                                      !    Dimensions: (ngptsw,ncol,nlay)
      real(kind=8), intent(out) :: ssacmcl(:,:,:)    ! in-cloud single scattering albedo [mcica]
                                                      !    Dimensions: (ngptsw,ncol,nlay)
      real(kind=8), intent(out) :: asmcmcl(:,:,:)    ! in-cloud asymmetry parameter [mcica]
                                                      !    Dimensions: (ngptsw,ncol,nlay)
      real(kind=8), intent(out) :: fsfcmcl(:,:,:)    ! in-cloud forward scattering fraction [mcica]
                                                      !    Dimensions: (ngptsw,ncol,nlay)

! ----- Local -----

! Stochastic cloud generator variables [mcica]
      integer(kind=4), parameter :: nsubcsw = ngptsw ! number of sub-columns (g-point intervals)
      integer(kind=4) :: ilev                        ! loop index

      real(kind=8) :: pmid(ncol,nlay)                ! layer pressures (Pa) 
!      real(kind=8) :: pdel(ncol,nlay)               ! layer pressure thickness (Pa) 
!      real(kind=8) :: qi(ncol,nlay)                 ! ice water (specific humidity)
!      real(kind=8) :: ql(ncol,nlay)                 ! liq water (specific humidity)
...
! Convert pressures from mb to Pa
      ...
      pmid(:ncol,:nlay) = play(:ncol,:nlay)*1.e2_8
...
!      do ilev = 1, nlay
!         qi(ilev) = (ciwp(ilev) * grav) / (pdel(ilev) * 1000._8)
!         ql(ilev) = (clwp(ilev) * grav) / (pdel(ilev) * 1000._8)
!      enddo
...
! -- Arguments
      ...
      integer(kind=4), intent(in) :: ncol            ! number of layers
      integer(kind=4), intent(in) :: nlay            ! number of layers
      integer(kind=4), intent(in) :: icld            ! clear/cloud, cloud overlap flag
      integer(kind=4), intent(inout) :: irng         ! flag for random number generator
                                                      !  0 = kissvec
                                                      !  1 = Mersenne Twister
      integer(kind=4), intent(in) :: nsubcol         ! number of sub-columns (g-point intervals)
      integer(kind=4), optional, intent(in) :: changeSeed     ! allows permuting seed

! Column state (cloud fraction, cloud water, cloud ice) + variables needed to read physics state 
      real(kind=8), intent(in) :: pmid(:,:)          ! layer pressure (Pa)
                                                      !    Dimensions: (ncol,nlay)
      real(kind=8), intent(in) :: cld(:,:)           ! cloud fraction 
                                                      !    Dimensions: (ncol,nlay)
      real(kind=8), intent(in) :: clwp(:,:)          ! in-cloud liquid water path (g/m2)
                                                      !    Dimensions: (ncol,nlay)
      real(kind=8), intent(in) :: ciwp(:,:)          ! in-cloud ice water path (g/m2)
                                                      !    Dimensions: (ncol,nlay)
      real(kind=8), intent(in) :: tauc(:,:,:)        ! in-cloud optical depth (non-delta scaled)
                                                      !    Dimensions: (nbndsw,ncol,nlay)
      real(kind=8), intent(in) :: ssac(:,:,:)        ! in-cloud single scattering albedo (non-delta scaled)
                                                      !    Dimensions: (nbndsw,ncol,nlay)
      real(kind=8), intent(in) :: asmc(:,:,:)        ! in-cloud asymmetry parameter (non-delta scaled)
      ...
      real(kind=8), intent(out) :: cld_stoch(:,:,:)  ! subcolumn cloud fraction 
                                                      !    Dimensions: (ngptsw,ncol,nlay)
      real(kind=8), intent(out) :: clwp_stoch(:,:,:) ! subcolumn in-cloud liquid water path
                                                      !    Dimensions: (ngptsw,ncol,nlay)
      real(kind=8), intent(out) :: ciwp_stoch(:,:,:) ! subcolumn in-cloud ice water path
                                                      !    Dimensions: (ngptsw,ncol,nlay)
      real(kind=8), intent(out) :: tauc_stoch(:,:,:) ! subcolumn in-cloud optical depth
                                                      !    Dimensions: (ngptsw,ncol,nlay)
      real(kind=8), intent(out) :: ssac_stoch(:,:,:) ! subcolumn in-cloud single scattering albedo
                                                      !    Dimensions: (ngptsw,ncol,nlay)
      real(kind=8), intent(out) :: asmc_stoch(:,:,:) ! subcolumn in-cloud asymmetry parameter
                                                      !    Dimensions: (ngptsw,ncol,nlay)
      real(kind=8), intent(out) :: fsfc_stoch(:,:,:) ! subcolumn in-cloud forward scattering fraction
                                                      !    Dimensions: (ngptsw,ncol,nlay)

! -- Local variables
      real(kind=8) :: cldf(ncol,nlay)                ! cloud fraction 
                                                      !    Dimensions: (ncol,nlay)

! Mean over the subcolumns (cloud fraction, cloud water , cloud ice) - inactive
!      real(kind=8) :: mean_cld_stoch(ncol,nlay)     ! cloud fraction 
!      real(kind=8) :: mean_clwp_stoch(ncol,nlay)    ! cloud water
!      real(kind=8) :: mean_ciwp_stoch(ncol,nlay)    ! cloud ice
!      real(kind=8) :: mean_tauc_stoch(ncol,nlay)    ! cloud optical depth
!      real(kind=8) :: mean_ssac_stoch(ncol,nlay)    ! cloud single scattering albedo
!      real(kind=8) :: mean_asmc_stoch(ncol,nlay)    ! cloud asymmetry parameter
!      real(kind=8) :: mean_fsfc_stoch(ncol,nlay)    ! cloud forward scattering fraction

! Set overlap
      integer(kind=4) :: overlap                     ! 1 = random overlap, 2 = maximum/random,
                                                      ! 3 = maximum overlap, 
!      real(kind=8), parameter  :: Zo = 2500._8        ! length scale (m) 
!      real(kind=8) :: zm(ncon,nlay)                    ! Height of midpoints (above surface)
!      real(kind=8), dimension(nlay) :: alpha=0.0_8    ! overlap parameter  

! Constants (min value for cloud fraction and cloud water and ice)
      real(kind=8), parameter :: cldmin = 1.0e-20_8 ! min cloud fraction
!      real(kind=8), parameter :: qmin   = 1.0e-10_8   ! min cloud water and cloud ice (not used)

! Variables related to random number and seed 
      real(kind=8), dimension(nsubcol, ncol, nlay) :: CDF, CDF2       ! random numbers
      integer(kind=4), dimension(ncol) :: seed1, seed2, seed3, seed4  ! seed to create random number
      real(kind=8), dimension(ncol) :: rand_num       ! random number (kissvec)
      integer(kind=4) :: iseed                        ! seed to create random number (Mersenne Twister)
      real(kind=8) :: rand_num_mt                     ! random number (Mersenne Twister)
...
! Indices
      integer(kind=4) :: ilev, isubcol, i, n, ngbm    ! indices
...
! Must use pmid from bottom four layers. 
         do i=1,ncol
            ...
            seed1(i) = (pmid(i,1) - int(pmid(i,1)))  * 1000000000_4
            seed2(i) = (pmid(i,2) - int(pmid(i,2)))  * 1000000000_4
            seed3(i) = (pmid(i,3) - int(pmid(i,3)))  * 1000000000_4
            seed4(i) = (pmid(i,4) - int(pmid(i,4)))  * 1000000000_4
          enddo
         do i=1,changeSeed
...
!    - if the layer above is clear, we use a new random number 
         ...
         do ilev = 2,nlay
            do i = 1, ncol
               do isubcol = 1, nsubcol
                  if (CDF(isubcol, i, ilev-1) > 1._8 - cldf(i,ilev-1) ) then
                     CDF(isubcol,i,ilev) = CDF(isubcol,i,ilev-1) 
                  else
                     CDF(isubcol,i,ilev) = CDF(isubcol,i,ilev) * (1._8 - cldf(i,ilev-1)) 
                  endif
               enddo
            enddo
         enddo
...
! -- generate subcolumns for homogeneous clouds -----
      do ilev = 1, nlay
         isCloudy(:,:,ilev) = (CDF(:,:,ilev) >= 1._8 - spread(cldf(:,ilev), dim=1, nCopies=nsubcol) )
      enddo
         ...
         do i = 1, ncol
            do isubcol = 1, nsubcol
               if ( iscloudy(isubcol,i,ilev) ) then
                  ...
                  cld_stoch(isubcol,i,ilev) = 0._8
                  clwp_stoch(isubcol,i,ilev) = 0._8
                  ciwp_stoch(isubcol,i,ilev) = 0._8
                  tauc_stoch(isubcol,i,ilev) = 0._8
                  ssac_stoch(isubcol,i,ilev) = 1._8
                  asmc_stoch(isubcol,i,ilev) = 0._8
                  fsfc_stoch(isubcol,i,ilev) = 0._8
               endif
            enddo
         enddo
      enddo
...
!      mean_cld_stoch(:,:) = 0._8
!      mean_clwp_stoch(:,:) = 0._8
!      mean_ciwp_stoch(:,:) = 0._8
!      mean_tauc_stoch(:,:) = 0._8
!      mean_ssac_stoch(:,:) = 0._8
!      mean_asmc_stoch(:,:) = 0._8
!      mean_fsfc_stoch(:,:) = 0._8
!      do i = 1, nsubcol
...
!
      real(kind=8), dimension(:), intent(inout)  :: ran_arr
      integer(kind=4), dimension(:), intent(inout) :: seed1,seed2,seed3,seed4
      integer(kind=4) :: i,sz,kiss
      integer(kind=4) :: m, k, n
...
! inline function 
      ...
      do i = 1, sz
         ...
         seed3(i) = 18000_4 * iand (seed3(i), 65535_4) + ishft (seed3(i), - 16_4)
         seed4(i) = 30903_4 * iand (seed4(i), 65535_4) + ishft (seed4(i), - 16_4)
         kiss = seed1(i) + seed2(i) + ishft (seed3(i), 16_4) + seed4(i)
         ran_arr(i) = kiss*2.328306e-10_8 + 0.5_8
      end do
