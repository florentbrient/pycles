...
! Declarations for radiative transfer
      ...
      real(kind=8) :: odepth, odtot, odepth_rec, odtot_rec, gassrc, ttot
...
! Declarations for cloud overlap adjustment
      real(kind=8) :: faccld1(nlayers+1),faccld2(nlayers+1)
      real(kind=8) :: facclr1(nlayers+1),facclr2(nlayers+1)
      real(kind=8) :: faccmb1(nlayers+1),faccmb2(nlayers+1)
      real(kind=8) :: faccld1d(0:nlayers),faccld2d(0:nlayers)
      real(kind=8) :: facclr1d(0:nlayers),facclr2d(0:nlayers)
      real(kind=8) :: faccmb1d(0:nlayers),faccmb2d(0:nlayers)

      real(kind=8) :: fmax, fmin, rat1, rat2
      real(kind=8) :: clrradd, cldradd, clrradu, cldradu, oldclr, oldcld
      real(kind=8) :: rad, cldsrc, radmod

      integer(kind=4) :: istcld(nlayers+1),istcldd(0:nlayers)
...
! Maximum/random cloud overlap
            ...
            elseif (cldfrac(lev+1) .ge. cldfrac(lev)) then
               ...
               if (istcld(lev) .eq. 1) then
                  facclr1(lev+1) = 0._8
                  facclr2(lev+1) = 0._8
                  if (cldfrac(lev) .lt. 1._8) facclr2(lev+1) = &
                     (cldfrac(lev+1)-cldfrac(lev))/(1._8-cldfrac(lev))
                  facclr2(lev) = 0._8
                  faccld2(lev) = 0._8
               else
                  ...
                  if (cldfrac(lev+1) .gt. fmax) then
                     facclr1(lev+1) = rat2
                     facclr2(lev+1) = (cldfrac(lev+1)-fmax)/(1._8-fmax)
               ...
               if (facclr1(lev+1).gt.0._8 .or. facclr2(lev+1).gt.0._8) then
                  rat1 = 1._8
                  rat2 = 0._8
               else
                  rat1 = 0._8
               ...
               if (faccld1(lev+1).gt.0._8 .or. faccld2(lev+1).gt.0._8) then
                  rat1 = 0._8
                  rat2 = 1._8
               else
                  rat1 = 0._8
            ...
            faccmb2(lev+1) = faccld1(lev+1) * facclr2(lev) * (1._8 - cldfrac(lev-1)) 
         else
            istcld(lev+1) = 1
         endif
      enddo
         ...
         if (icldlyr(lev).eq.1) then
            ...
            elseif (cldfrac(lev-1) .ge. cldfrac(lev)) then
               ...
               if (istcldd(lev) .eq. 1) then
                  facclr1d(lev-1) = 0._8
                  facclr2d(lev-1) = 0._8
                  if (cldfrac(lev) .lt. 1._8) facclr2d(lev-1) = &
                     (cldfrac(lev-1)-cldfrac(lev))/(1._8-cldfrac(lev))
                  facclr2d(lev) = 0._8
                  faccld2d(lev) = 0._8
               else
                  ...
                  if (cldfrac(lev-1) .gt. fmax) then
                     facclr1d(lev-1) = rat2
                     facclr2d(lev-1) = (cldfrac(lev-1)-fmax)/(1._8-fmax)
                  elseif (cldfrac(lev-1) .lt. fmax) then
               ...
               if (facclr1d(lev-1).gt.0._8 .or. facclr2d(lev-1).gt.0._8)then
                  rat1 = 1._8
                  rat2 = 0._8
               else
                  rat1 = 0._8
               ...
               if (faccld1d(lev-1).gt.0._8 .or. faccld2d(lev-1).gt.0._8)then
                  rat1 = 0._8
                  rat2 = 1._8
               else
                  rat1 = 0._8
            ...
            faccmb2d(lev-1) = faccld1d(lev-1) * facclr2d(lev) * (1._8 - cldfrac(lev+1))
         else
            istcldd(lev-1) = 1
         endif
      enddo
...
! Cloudy layer
               if (icldlyr(lev).eq.1) then
                  ...
                  clrradd = clrradd * (1._8-atrans(lev)) + (1._8 - cldfrac(lev)) * gassrc
...
! Cloudy layer
            if (icldlyr(lev) .eq. 1) then
               ...
               clrradu = clrradu * (1.0_8-atrans(lev)) + (1._8 - cldfrac(lev)) * gassrc
! Total sky radiance
               radlu = cldradu + clrradu
               urad(lev) = urad(lev) + radlu
               radmod = rad * &
                   (facclr1(lev+1)*(1.0_8-atrans(lev))+ &
                   faccld1(lev+1) *  ttot) - &
