...
! ----- Input -----
      ...
      real(kind=8), intent(in) :: pavel(nlayers)           ! layer pressures (mb) 
                                                      !    Dimensions: (nlayers)
      real(kind=8), intent(in) :: wx(maxxsec,nlayers)            ! cross-section amounts (mol/cm2)
                                                      !    Dimensions: (maxxsec,nlayers)
      real(kind=8), intent(in) :: coldry(nlayers)          ! column amount (dry air)
                                                      !    Dimensions: (nlayers)

      integer(kind=4), intent(in) :: laytrop         ! tropopause layer index
      ...
      real(kind=8), intent(in) :: colh2o(nlayers)          ! column amount (h2o)
                                                      !    Dimensions: (nlayers)
      real(kind=8), intent(in) :: colco2(nlayers)          ! column amount (co2)
                                                      !    Dimensions: (nlayers)
      real(kind=8), intent(in) :: colo3(nlayers)           ! column amount (o3)
                                                      !    Dimensions: (nlayers)
      real(kind=8), intent(in) :: coln2o(nlayers)          ! column amount (n2o)
                                                      !    Dimensions: (nlayers)
      real(kind=8), intent(in) :: colco(nlayers)           ! column amount (co)
                                                      !    Dimensions: (nlayers)
      real(kind=8), intent(in) :: colch4(nlayers)          ! column amount (ch4)
                                                      !    Dimensions: (nlayers)
      real(kind=8), intent(in) :: colo2(nlayers)           ! column amount (o2)
                                                      !    Dimensions: (nlayers)
      real(kind=8), intent(in) :: colbrd(nlayers)          ! column amount (broadening gases)
                                                      !    Dimensions: (nlayers)
      ...
      integer(kind=4), intent(in) :: indminor(nlayers)
                                                      !    Dimensions: (nlayers)
      real(kind=8), intent(in) :: minorfrac(nlayers)
                                                      !    Dimensions: (nlayers)
      real(kind=8), intent(in) :: scaleminor(nlayers)
                                                      !    Dimensions: (nlayers)
      real(kind=8), intent(in) :: scaleminorn2(nlayers)
                                                      !    Dimensions: (nlayers)
...
! ----- Output -----
      real(kind=8), intent(out) :: fracs(nlayers,ngptlw)        ! planck fractions
                                                      !    Dimensions: (nlayers,ngptlw)
      real(kind=8), intent(out) :: taug(nlayers,ngptlw)         ! gaseous optical depth 
                                                      !    Dimensions: (nlayers,ngptlw)
...
! Local 
      integer(kind=4) :: lay, ind0, ind1, inds, indf, indm, ig
      real(kind=8) :: pp, corradj, scalen2, tauself, taufor, taun2
...
! Lower atmosphere loop
      do lay = 1, laytrop
         ...
         pp = pavel(lay)
         corradj =  1.
         if (pp .lt. 250._8) then
            corradj = 1._8 - 0.15_8 * (250._8-pp) / 154.4_8
         endif
...
! Local 
      integer(kind=4) :: lay, ind0, ind1, inds, indf, ig
      real(kind=8) :: pp, corradj, tauself, taufor
...
! Lower atmosphere loop
      do lay = 1, laytrop
         ...
         indf = indfor(lay)
         pp = pavel(lay)
         corradj = 1._8 - .05_8 * (pp - 100._8) / 900._8
         do ig = 1, ng2
            tauself = selffac(lay) * (selfref(inds,ig) + selffrac(lay) * &
                 (selfref(inds+1,ig) - selfref(inds,ig)))
            taufor =  forfac(lay) * (forref(indf,ig) + forfrac(lay) * &
...
! Local 
      integer(kind=4) :: lay, ind0, ind1, inds, indf, indm, ig
      integer(kind=4) :: js, js1, jmn2o, jpl
      real(kind=8) :: speccomb, specparm, specmult, fs
      real(kind=8) :: speccomb1, specparm1, specmult1, fs1
      real(kind=8) :: speccomb_mn2o, specparm_mn2o, specmult_mn2o, &
                       fmn2o, fmn2omf, chi_n2o, ratn2o, adjfac, adjcoln2o
      real(kind=8) :: speccomb_planck, specparm_planck, specmult_planck, fpl
      real(kind=8) :: p, p4, fk0, fk1, fk2
      real(kind=8) :: fac000, fac100, fac200, fac010, fac110, fac210
      real(kind=8) :: fac001, fac101, fac201, fac011, fac111, fac211
      real(kind=8) :: tauself, taufor, n2om1, n2om2, absn2o
      real(kind=8) :: refrat_planck_a, refrat_planck_b, refrat_m_a, refrat_m_b
      real(kind=8) :: tau_major, tau_major1
...
! Lower atmosphere loop
      do lay = 1, laytrop
         ...
         specmult = 8._8*(specparm)
         js = 1 + int(specmult)
         fs = mod(specmult,1.0_8)        
         ...
         specmult1 = 8._8*(specparm1)
         js1 = 1 + int(specmult1)
         fs1 = mod(specmult1,1.0_8)
         ...
         specmult_mn2o = 8._8*specparm_mn2o
         jmn2o = 1 + int(specmult_mn2o)
         fmn2o = mod(specmult_mn2o,1.0_8)
         fmn2omf = minorfrac(lay)*fmn2o
!  In atmospheres where the amount of N2O is too great to be considered
!  a minor species, adjust the column amount of N2O by an empirical factor 
!  to obtain the proper contribution.
         chi_n2o = coln2o(lay)/coldry(lay)
         ratn2o = 1.e20_8*chi_n2o/chi_mls(4,jp(lay)+1)
         if (ratn2o .gt. 1.5_8) then
            adjfac = 0.5_8+(ratn2o-0.5_8)**0.65_8
            adjcoln2o = adjfac*chi_mls(4,jp(lay)+1)*coldry(lay)*1.e-20_8
         else
         ...
         specmult_planck = 8._8*specparm_planck
         jpl= 1 + int(specmult_planck)
         fpl = mod(specmult_planck,1.0_8)
         ...
         if (specparm .lt. 0.125_8) then
         ...
         else if (specparm .gt. 0.875_8) then
            p = -fs 
            p4 = p**4
            fk0 = p4
            fk1 = 1 - p - 2.0_8*p4
            ...
            fac000 = (1._8 - fs) * fac00(lay)
            fac010 = (1._8 - fs) * fac10(lay)
            fac100 = fs * fac00(lay)
            fac110 = fs * fac10(lay)
         endif
         if (specparm1 .lt. 0.125_8) then
            p = fs1 - 1
            p4 = p**4
            fk0 = p4
            fk1 = 1 - p - 2.0_8*p4
         ...
         else if (specparm1 .gt. 0.875_8) then
            p = -fs1 
            p4 = p**4
            fk0 = p4
            fk1 = 1 - p - 2.0_8*p4
            ...
            fac001 = (1._8 - fs1) * fac01(lay)
            fac011 = (1._8 - fs1) * fac11(lay)
...
! Upper atmosphere loop
      do lay = laytrop+1, nlayers
         ...
         specparm_mn2o = colh2o(lay)/speccomb_mn2o
         if (specparm_mn2o .ge. oneminus) specparm_mn2o = oneminus
         specmult_mn2o = 4._8*specparm_mn2o
         jmn2o = 1 + int(specmult_mn2o)
         fmn2o = mod(specmult_mn2o,1.0_8)
         fmn2omf = minorfrac(lay)*fmn2o
!  In atmospheres where the amount of N2O is too great to be considered
...
!  to obtain the proper contribution.
         ...
         specparm_planck = colh2o(lay)/speccomb_planck
         if (specparm_planck .ge. oneminus) specparm_planck=oneminus
         specmult_planck = 4._8*specparm_planck
         jpl= 1 + int(specmult_planck)
         fpl = mod(specmult_planck,1.0_8)
...
! Local 
      ...
      real(kind=8) :: fac000, fac100, fac200, fac010, fac110, fac210
      real(kind=8) :: fac001, fac101, fac201, fac011, fac111, fac211
      real(kind=8) :: tauself, taufor
      real(kind=8) :: refrat_planck_a, refrat_planck_b
      real(kind=8) :: tau_major, tau_major1
...
! Local 
      integer(kind=4) :: lay, ind0, ind1, inds, indf, indm, ig
      integer(kind=4) :: js, js1, jmo3, jpl
      real(kind=8) :: speccomb, specparm, specmult, fs
      real(kind=8) :: speccomb1, specparm1, specmult1, fs1
      real(kind=8) :: speccomb_mo3, specparm_mo3, specmult_mo3, fmo3
      real(kind=8) :: speccomb_planck, specparm_planck, specmult_planck, fpl
      real(kind=8) :: p, p4, fk0, fk1, fk2
      real(kind=8) :: fac000, fac100, fac200, fac010, fac110, fac210
      real(kind=8) :: fac001, fac101, fac201, fac011, fac111, fac211
      real(kind=8) :: tauself, taufor, o3m1, o3m2, abso3
      real(kind=8) :: refrat_planck_a, refrat_planck_b, refrat_m_a
      real(kind=8) :: tau_major, tau_major1
...
! Lower atmosphere loop
      do lay = 1, laytrop
         ...
         specmult_mo3 = 8._8*specparm_mo3
         jmo3 = 1 + int(specmult_mo3)
         fmo3 = mod(specmult_mo3,1.0_8)
...
! Local 
      integer(kind=4) :: lay, ind0, ind1, inds, indf, indm, ig
      real(kind=8) :: chi_co2, ratco2, adjfac, adjcolco2
      real(kind=8) :: tauself, taufor, absco2
...
! to obtain the proper contribution.
         ...
         ratco2 = 1.e20_8*chi_co2/chi_mls(2,jp(lay)+1)
         if (ratco2 .gt. 3.0_8) then
            adjfac = 2.0_8+(ratco2-2.0_8)**0.77_8
            adjcolco2 = adjfac*chi_mls(2,jp(lay)+1)*coldry(lay)*1.e-20_8
         else
...
! Local 
      integer(kind=4) :: lay, ind0, ind1, inds, indf, indm, ig
      integer(kind=4) :: js, js1, jmco2, jpl
      real(kind=8) :: speccomb, specparm, specmult, fs
      real(kind=8) :: speccomb1, specparm1, specmult1, fs1
      real(kind=8) :: speccomb_mco2, specparm_mco2, specmult_mco2, fmco2
      real(kind=8) :: speccomb_planck, specparm_planck, specmult_planck, fpl
      real(kind=8) :: p, p4, fk0, fk1, fk2
      real(kind=8) :: fac000, fac100, fac200, fac010, fac110, fac210
      real(kind=8) :: fac001, fac101, fac201, fac011, fac111, fac211
      real(kind=8) :: tauself, taufor, co2m1, co2m2, absco2
      real(kind=8) :: chi_co2, ratco2, adjfac, adjcolco2
      real(kind=8) :: refrat_planck_a, refrat_m_a
      real(kind=8) :: tau_major, tau_major1
...
! Lower atmosphere loop
      do lay = 1, laytrop
         ...
         specmult_mco2 = 8._8*specparm_mco2

         jmco2 = 1 + int(specmult_mco2)
         fmco2 = mod(specmult_mco2,1.0_8)
...
!  to obtain the proper contribution.
         ...
         if (ratco2 .gt. 3.0_8) then
            adjfac = 3.0_8+(ratco2-3.0_8)**0.79_8
            adjcolco2 = adjfac*chi_mls(2,jp(lay)+1)*coldry(lay)*1.e-20_8
         else
            adjcolco2 = colco2(lay)
         endif
...
!  to obtain the proper contribution.
         ...
         ratco2 = 1.e20*chi_co2/chi_mls(2,jp(lay)+1)
         if (ratco2 .gt. 3.0_8) then
            adjfac = 2.0_8+(ratco2-2.0_8)**0.79_8
            adjcolco2 = adjfac*chi_mls(2,jp(lay)+1)*coldry(lay)*1.e-20_8
         else
            adjcolco2 = colco2(lay)
         endif
...
! for o3.  Revised to apply weighting for g-point reduction in this band.
         ...
         taug(lay,ngs6+6)=taug(lay,ngs6+6)*0.92_8
         taug(lay,ngs6+7)=taug(lay,ngs6+7)*0.88_8
         taug(lay,ngs6+8)=taug(lay,ngs6+8)*1.07_8
         taug(lay,ngs6+9)=taug(lay,ngs6+9)*1.1_8
         taug(lay,ngs6+10)=taug(lay,ngs6+10)*0.99_8
         taug(lay,ngs6+11)=taug(lay,ngs6+11)*0.855_8
...
! Local 
      integer(kind=4) :: lay, ind0, ind1, inds, indf, indm, ig
      real(kind=8) :: tauself, taufor, absco2, abso3, absn2o
      real(kind=8) :: chi_co2, ratco2, adjfac, adjcolco2
...
!  to obtain the proper contribution.
         ...
         ratco2 = 1.e20_8*chi_co2/chi_mls(2,jp(lay)+1)
         if (ratco2 .gt. 3.0_8) then
            adjfac = 2.0_8+(ratco2-2.0_8)**0.65_8
            adjcolco2 = adjfac*chi_mls(2,jp(lay)+1)*coldry(lay)*1.e-20_8
         else
            adjcolco2 = colco2(lay)
         endif
...
! Local 
      ...
      real(kind=8) :: speccomb, specparm, specmult, fs
      real(kind=8) :: speccomb1, specparm1, specmult1, fs1
      real(kind=8) :: speccomb_mn2o, specparm_mn2o, specmult_mn2o, fmn2o
      real(kind=8) :: speccomb_planck, specparm_planck, specmult_planck, fpl
      real(kind=8) :: p, p4, fk0, fk1, fk2
      real(kind=8) :: fac000, fac100, fac200, fac010, fac110, fac210
      real(kind=8) :: fac001, fac101, fac201, fac011, fac111, fac211
      real(kind=8) :: tauself, taufor, n2om1, n2om2, absn2o
      real(kind=8) :: chi_n2o, ratn2o, adjfac, adjcoln2o
      real(kind=8) :: refrat_planck_a, refrat_m_a
      real(kind=8) :: tau_major, tau_major1
...
! Local 
      integer(kind=4) :: lay, ind0, ind1, inds, indf, indm, ig
      real(kind=8) :: scaleo2, tauself, taufor, tauo2
...
! Upper atmosphere loop
      do lay = laytrop+1, nlayers
         do ig = 1, ng12
            taug(lay,ngs11+ig) = 0.0_8
            fracs(lay,ngs11+ig) = 0.0_8
         enddo
      enddo
...
! Local 
      integer(kind=4) :: lay, ind0, ind1, inds, indf, indm, ig
      integer(kind=4) :: js, js1, jmco2, jmco, jpl
      real(kind=8) :: speccomb, specparm, specmult, fs
      real(kind=8) :: speccomb1, specparm1, specmult1, fs1
      real(kind=8) :: speccomb_mco2, specparm_mco2, specmult_mco2, fmco2
      real(kind=8) :: speccomb_mco, specparm_mco, specmult_mco, fmco
      real(kind=8) :: speccomb_planck, specparm_planck, specmult_planck, fpl
      real(kind=8) :: p, p4, fk0, fk1, fk2
      real(kind=8) :: fac000, fac100, fac200, fac010, fac110, fac210
      real(kind=8) :: fac001, fac101, fac201, fac011, fac111, fac211
      real(kind=8) :: tauself, taufor, co2m1, co2m2, absco2 
      real(kind=8) :: com1, com2, absco, abso3
      real(kind=8) :: chi_co2, ratco2, adjfac, adjcolco2
      real(kind=8) :: refrat_planck_a, refrat_m_a, refrat_m_a3
      real(kind=8) :: tau_major, tau_major1
...
!  to obtain the proper contribution.
         chi_co2 = colco2(lay)/(coldry(lay))
         ratco2 = 1.e20_8*chi_co2/3.55e-4_8
         if (ratco2 .gt. 3.0_8) then
            adjfac = 2.0_8+(ratco2-2.0_8)**0.68_8
            adjcolco2 = adjfac*3.55e-4*coldry(lay)*1.e-20_8
         else
            adjcolco2 = colco2(lay)
         endif
         ...
         specmult_mco = 8._8*specparm_mco
         jmco = 1 + int(specmult_mco)
         fmco = mod(specmult_mco,1.0_8)
...
! Local 
      integer(kind=4) :: lay, ind0, ind1, inds, indf, indm, ig
      integer(kind=4) :: js, js1, jmn2, jpl
      real(kind=8) :: speccomb, specparm, specmult, fs
      real(kind=8) :: speccomb1, specparm1, specmult1, fs1
      real(kind=8) :: speccomb_mn2, specparm_mn2, specmult_mn2, fmn2
      real(kind=8) :: speccomb_planck, specparm_planck, specmult_planck, fpl
      real(kind=8) :: p, p4, fk0, fk1, fk2
      real(kind=8) :: fac000, fac100, fac200, fac010, fac110, fac210
      real(kind=8) :: fac001, fac101, fac201, fac011, fac111, fac211
      real(kind=8) :: scalen2, tauself, taufor, n2m1, n2m2, taun2 
      real(kind=8) :: refrat_planck_a, refrat_m_a
      real(kind=8) :: tau_major, tau_major1
...
! Lower atmosphere loop
      do lay = 1, laytrop
         ...
         specmult_mn2 = 8._8*specparm_mn2
         jmn2 = 1 + int(specmult_mn2)
         fmn2 = mod(specmult_mn2,1.0_8)
...
! Upper atmosphere loop
      do lay = laytrop+1, nlayers
         do ig = 1, ng15
            taug(lay,ngs14+ig) = 0.0_8
            fracs(lay,ngs14+ig) = 0.0_8
         enddo
      enddo
