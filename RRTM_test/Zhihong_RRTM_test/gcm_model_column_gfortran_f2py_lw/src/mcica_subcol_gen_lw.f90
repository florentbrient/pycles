...
! ----- Input -----
! Control
      integer(kind=4), intent(in) :: iplon           ! column/longitude index
...
!      real(kind=8), intent(in) :: ssac(:,:,:)       ! in-cloud single scattering albedo
                                                      !    Dimensions: (nbndlw,ncol,nlay)
!      real(kind=8), intent(in) :: asmc(:,:,:)       ! in-cloud asymmetry parameter
                                                      !    Dimensions: (nbndlw,ncol,nlay)
...
! Stochastic cloud generator variables [mcica]
      integer(kind=4), parameter :: nsubclw = ngptlw ! number of sub-columns (g-point intervals)
...
!      real(kind=8) :: zm(ncol,nlay)                 ! Height of midpoints (above surface)
...
! Variables related to random number and seed 
      ...
      integer(kind=4), dimension(ncol) :: seed1, seed2, seed3, seed4 ! seed to create random number (kissvec)
      real(kind=8), dimension(ncol) :: rand_num      ! random number (kissvec)
      integer(kind=4) :: iseed                       ! seed to create random number (Mersenne Teister)
...
! Indices
      integer(kind=4) :: ilev, isubcol, i, n         ! indices
...
!                  asmc_stoch(isubcol,i,ilev) = 1._8
               endif
            enddo
         enddo
      enddo
