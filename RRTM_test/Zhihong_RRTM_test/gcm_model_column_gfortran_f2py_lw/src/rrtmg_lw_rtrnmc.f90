...
! Clouds
      ...
      real(kind=8), intent(in) :: cldfmc(ngptlw,nlayers)        ! layer cloud fraction [mcica]
                                                      !    Dimensions: (ngptlw,nlayers)
      real(kind=8), intent(in) :: taucmc(ngptlw,nlayers)        ! layer cloud optical depth [mcica]
                                                      !    Dimensions: (ngptlw,nlayers)
...
! Declarations for radiative transfer
      real(kind=8) :: abscld(nlayers,ngptlw)
      ...
      real(kind=8) :: efclfrac(nlayers,ngptlw)
      ...
      real(kind=8) :: odcld(nlayers,ngptlw)
      ...
      integer(kind=4) :: ibnd, ib, iband, lay, lev, l, ig  ! loop indices
...
!  Cloudy layer
               if (icldlyr(lev).eq.1) then
                  ...
                  elseif (odepth .le. 0.06_8) then
                     ...
                     radld = radld - radld * (atrans(lev) + &
                         efclfrac(lev,igc) * (1._8 - atrans(lev))) + &
                         gassrc + cldfmc(igc,lev) * &
                         (bbdtot * atot(lev) - gassrc)
                     drad(lev-1) = drad(lev-1) + radld
...
!  Cloudy layer
            if (icldlyr(lev) .eq. 1) then
               ...
               if (idrv .eq. 1) then
                  d_radlu_dt = d_radlu_dt * cldfmc(igc,lev) * (1.0_8 - atot(lev)) + &
                         d_radlu_dt * (1.0_8 - cldfmc(igc,lev)) * (1.0_8 - atrans(lev))
