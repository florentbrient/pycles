...
! ----- Output -----
      real(kind=8), intent(out) :: sfluxzen(ngptsw)          ! solar source function
                                                         !   Dimensions: (ngptsw)
      real(kind=8), intent(out) :: taug(nlayers,ngptsw)            ! gaseous optical depth 
                                                         !   Dimensions: (nlayers,ngptsw)
      real(kind=8), intent(out) :: taur(nlayers,ngptsw)            ! Rayleigh 
                                                         !   Dimensions: (nlayers,ngptsw)
!      real(kind=8), intent(out) :: ssa(nlayers,ngptsw)            ! single scattering albedo (inactive)
                                                         !   Dimensions: (nlayers,ngptsw)
...
! Local
      ...
      integer(kind=4) :: ig, ind0, ind1, inds, indf, js, lay, laysolfr, &
                          layreffr
      real(kind=8) :: fac000, fac001, fac010, fac011, fac100, fac101, &
...
! Lower atmosphere loop
      do lay = 1, laytrop
         ...
         js = 1 + int(specmult)
         fs = mod(specmult, 1._8 )
         fac000 = (1._8 - fs) * fac00(lay)
         fac010 = (1._8 - fs) * fac10(lay)
         fac100 = fs * fac00(lay)
         fac110 = fs * fac10(lay)
         fac001 = (1._8 - fs) * fac01(lay)
         fac011 = (1._8 - fs) * fac11(lay)
         fac101 = fs * fac01(lay)
         fac111 = fs * fac11(lay)
...
! Lower atmosphere loop
      do lay = 1, laytrop
         if (jp(lay) .lt. layreffr .and. jp(lay+1) .ge. layreffr) &
            laysolfr = min(lay+1,laytrop)
         o2cont = 4.35e-4_8*colo2(lay)/(350.0_8*2.0_8)
         speccomb = colh2o(lay) + o2adj*strrat*colo2(lay)
         specparm = colh2o(lay)/speccomb 
         if (specparm .ge. oneminus) specparm = oneminus
         specmult = 8._8*(specparm)
!         odadj = specparm + o2adj * (1._8 - specparm)
         js = 1 + int(specmult)
         fs = mod(specmult, 1._8 )
         fac000 = (1._8 - fs) * fac00(lay)
         fac010 = (1._8 - fs) * fac10(lay)
...
!            ssa(lay,ngs22+ig) = 1.0_8
            taug(lay,ngs22+ig) = 0._8
