...
! ------- Input -------
      ...
      real(kind=8), intent(in) :: pgg(nlayers)                      ! asymmetry parameter
                                                               !   Dimensions: (nlayers)
      real(kind=8), intent(in) :: ptau(nlayers)                     ! optical depth
                                                               !   Dimensions: (nlayers)
      real(kind=8), intent(in) :: pw(nlayers)                       ! single scattering albedo 
                                                               !   Dimensions: (nlayers)
      real(kind=8), intent(in) :: prmuz                       ! cosine of solar zenith angle

! ------- Output -------

      real(kind=8), intent(inout) :: pref(nlayers+1)                  ! direct beam reflectivity
                                                               !   Dimensions: (nlayers+1)
      real(kind=8), intent(inout) :: prefd(nlayers+1)                 ! diffuse beam reflectivity
                                                               !   Dimensions: (nlayers+1)
      real(kind=8), intent(inout) :: ptra(nlayers+1)                  ! direct beam transmissivity
                                                               !   Dimensions: (nlayers+1)
      real(kind=8), intent(inout) :: ptrad(nlayers+1)                 ! diffuse beam transmissivity
                                                               !   Dimensions: (nlayers+1)

! ------- Local -------

      integer(kind=4) :: jk, jl, kmodts
      integer(kind=4) :: itind

      real(kind=8) :: tblind
      real(kind=8) :: za, za1, za2
      real(kind=8) :: zbeta, zdend, zdenr, zdent
      real(kind=8) :: ze1, ze2, zem1, zem2, zemm, zep1, zep2
      real(kind=8) :: zg, zg3, zgamma1, zgamma2, zgamma3, zgamma4, zgt
      real(kind=8) :: zr1, zr2, zr3, zr4, zr5
      real(kind=8) :: zrk, zrk2, zrkg, zrm1, zrp, zrp1, zrpp
      real(kind=8) :: zsr3, zt1, zt2, zt3, zt4, zt5, zto1
      real(kind=8) :: zw, zwcrit, zwo

      real(kind=8), parameter :: eps = 1.e-08_8
...
! General two-stream expressions
            ...
            zg3= 3._8 * zg
            if (kmodts == 1) then
               zgamma1= (7._8 - zw * (4._8 + zg3)) * 0.25_8
               zgamma2=-(1._8 - zw * (4._8 - zg3)) * 0.25_8
               zgamma3= (2._8 - zg3 * prmuz ) * 0.25_8
            else if (kmodts == 2) then  
               zgamma1= (8._8 - zw * (5._8 + zg3)) * 0.25_8
               zgamma2=  3._8 *(zw * (1._8 - zg )) * 0.25_8
               zgamma3= (2._8 - zg3 * prmuz ) * 0.25_8
            else if (kmodts == 3) then  
               zgamma1= zsr3 * (2._8 - zw * (1._8 + zg)) * 0.5_8
               zgamma2= zsr3 * zw * (1._8 - zg ) * 0.5_8
               zgamma3= (1._8 - zsr3 * zg * prmuz ) * 0.5_8
            end if
            zgamma4= 1._8 - zgamma3
    
! Recompute original s.s.a. to test for conservative solution

            zwo= zw / (1._8 - (1._8 - zw) * (zg / (1._8 - zg))**2)
...
!
               ...
               pref(jk) = (zgt - za1 * (1._8 - ze2)) / (1._8 + zgt)
               ptra(jk) = 1._8 - pref(jk)

! isotropic incidence

               prefd(jk) = zgt / (1._8 + zgt)
               ptrad(jk) = 1._8 - prefd(jk)        
...
! Homogeneous reflectance and transmittance
               ...
               ze1 = min ( zrk * zto1, 500._8)
               ze2 = min ( zto1 / prmuz , 500._8)
!
! Original
!              zep1 = exp( ze1 )
!              zem1 = exp(-ze1 )
...
! diffuse beam
               ...
               zemm = zem1*zem1
               zdend = 1._8 / ( (1._8 - zbeta*zemm ) * zrkg)
               prefd(jk) =  zgamma2 * (1._8 - zemm) * zdend
