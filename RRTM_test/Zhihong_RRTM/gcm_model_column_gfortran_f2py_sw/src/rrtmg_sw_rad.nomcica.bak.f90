...
! Atmosphere/clouds - cldprop
      ...
      real(kind=8) :: tauc(nbndsw,nlay+1)    ! in-cloud optical depth (non-delta scaled)
      real(kind=8) :: ssac(nbndsw,nlay+1)    ! in-cloud single scattering albedo (non-delta scaled)
      real(kind=8) :: asmc(nbndsw,nlay+1)    ! in-cloud asymmetry parameter (non-delta scaled)
      real(kind=8) :: fsfc(nbndsw,nlay+1)    ! in-cloud forward scattering fraction (non-delta scaled)
      real(kind=8) :: ciwp(nlay+1)           ! in-cloud ice water path
      real(kind=8) :: clwp(nlay+1)           ! in-cloud liquid water path
      real(kind=8) :: rel(nlay+1)            ! cloud liquid particle effective radius (microns)
      real(kind=8) :: rei(nlay+1)            ! cloud ice particle effective size (microns)
...
!  Stop model if partial cloudiness is present.
         ...
         ! DEBUG
         !write(*,*) 'tauc', tauc(:,4:5)
         write(*,*) 'taucldorig', taucldorig(3:6,:)
         !write(*,*) 'taucloud', taucloud(4:5,:)
         !write(*,*) 'ssacloud', ssacloud(4:5,:)
         !write(*,*) 'asmcloud', asmcloud(4:5,:)
